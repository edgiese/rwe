...
// ------------------------------------------------------------------------------
function ezIsRoom($height){
  // this function will return TRUE if there is sufficient room to output text totalling height or False if not
  return ($this->y-$height >= $this->ez['bottomMargin']);
}

function ezText($text,$size=0,$options=array(),$bCalcHeightOnly=False){
  ...
  // if $bCalcHeightOnly is set then the function does no output but returns the height for the text
  // otherwise, it returns the new 'y' position on the page
  
  // apply the filtering which will make the underlining function.
  $text = $this->ezProcessText($text);
  
  $newPage=false;
  $store_y=$starting_y=$this->y;
  $accum_height=0;	// amount accumulated from previous pages
  ...
  foreach ($lines as $line){
    ...
    while (strlen($line) || $start){
      ...
      if ($this->y < $this->ez['bottomMargin']){
        if ($bCalcHeightOnly){
          $newPage=true;
          // simlate a new page for y without putting it out
          $this->accum_height += $starting_y-$this->y;
          $this->y=$this->ez['pageHeight']-$this->ez['topMargin'];
          $starting_y=$this->y;
        } else {
          $this->ezNewPage();
      ...
      $line=$this->addTextWrap($left,$this->y,$right-$left,$size,$line,$just,0,$bCalcHeightOnly);
    }
  }

  if ($bCalcHeightOnly){
  	// return total calculated height
  	$retval=$starting_y-$this->y+$accum_height;
    $this->y=$store_y;
    return $retval;
  } else {
    return $this->y;
